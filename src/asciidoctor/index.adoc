...
The mapping is basically the same. Except that Foo implements **Identifiable** However, we define it inside **FooRepository** which implements **EntityActions[Foo]**.
